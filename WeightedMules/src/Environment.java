...
public class Environment {
	...
	public static int NUM_Mules;
	public static int NumberOfFailures;
	public static int NumberOfFailDurations;
	public static int MAX_X;
	public static int MAX_Y;
	public static int MAX_W; // weight
	...
	public ArrayList<Pointxy>  MuleLocationsEMS=null; //ems
	...
	// private HungarianAlgorithm ha;//used for optimally solving the assignment
	// problem
	// private int[] matchMuleToNode;//used for optimally solving the assignment
	// problem

	public Environment() { // HY added to change ranges
		NUM_Nodes = Main.NUM_Nodes;
		NUM_Mules = Main.NUM_Mules;
		NumberOfFailures = Main.NumberOfFailures;
		NumberOfFailDurations = Main.NumberOfFailDurations;
		MAX_X = Main.MAX_X;
		MAX_Y = Main.MAX_Y;
		MAX_W = Main.NUM_Nodes; // weight
		...
		if (Main.UniformFailureSettingMode) {
			setFailures();
		} else {
			setFailures1();
		}
	...
	public void placeNodes() {
		...
		if (Main.NodeGridMode) {
			if (NUM_Nodes == MAX_X * MAX_Y) {// number of nodes must be equal to
				// x*y
				...
				for (int loopx = 0; loopx <= (int) Math.sqrt(NUM_Nodes); loopx++) {
					for (int loopy = 0; loopy <= (int) Math.sqrt(NUM_Nodes); loopy++) {
						if (nodes.size() < Main.NUM_Nodes) {
							nodes.add(new Node(loopx * MAX_X / Math.sqrt(NUM_Nodes),
									loopy * MAX_Y / Math.sqrt(NUM_Nodes), this));// int
							// for
							// debug
						}
					}
			...
			for (int loopi = 0; loopi < NUM_Nodes; loopi++) {
				// nodes.add(new
				// Node((int)(rand.nextDouble()*MAX_X),(int)(rand.nextDouble()*MAX_Y),
				// this));//int for debug
				int x = (int) (rand.nextDouble() * MAX_X);
				int y = (int) (rand.nextDouble() * MAX_Y);
				int weight = 1;
				;
				if (Main.WeightedMode) {
					weight = (int) (rand.nextDouble() * MAX_W + 1);
				}
	...
	public void placeMules() {
		...
		System.out.println("centroid "+Main.CentroidAdjustmentMode);

		mules = new ArrayList<Mule>();
		Mule.indexCount = 1;
		MuleLocationsEMS = new ArrayList<Pointxy>();

		placedMules = new ArrayList<Mule>();
		if (Main.MuleManualSettingMode) {
			System.out.println("MuleMannualSettingMode=" + Main.MuleManualSettingMode);
			...
			//			MuleLocationsEMS.add(new Pointxy(2.5,2.5,0));
			//			MuleLocationsEMS.add(new Pointxy(2.5,7.5,1));
			//			MuleLocationsEMS.add(new Pointxy(7.5,2.5,2));
			//			MuleLocationsEMS.add(new Pointxy(7.5,7.5,3));

			MuleLocationsEMS.add(new Pointxy(MAX_X/6,MAX_Y/6));
			MuleLocationsEMS.add(new Pointxy(3*MAX_X/6,MAX_Y/6));
			MuleLocationsEMS.add(new Pointxy(5*MAX_X/6,MAX_Y/6));
			MuleLocationsEMS.add(new Pointxy(MAX_X/8,MAX_Y/2));
			MuleLocationsEMS.add(new Pointxy(3*MAX_X/8,MAX_Y/2));
			MuleLocationsEMS.add(new Pointxy(5*MAX_X/8,MAX_Y/2));
			MuleLocationsEMS.add(new Pointxy(7*MAX_X/8,MAX_Y/2));
			MuleLocationsEMS.add(new Pointxy(MAX_X/6,5*MAX_Y/6));
			MuleLocationsEMS.add(new Pointxy(3*MAX_X/6,5*MAX_Y/6));
			MuleLocationsEMS.add(new Pointxy(5*MAX_X/6,5*MAX_Y/6));
			Mule m;
			for(int i=0;i<MuleLocationsEMS.size();i++) {
				m =new Mule(MuleLocationsEMS.get(i),this);
				m.EMSLocationIndex=i;
				System.out.println("MannualSettingMode=true adding "+m);

				mules.add(m);
				MuleLocationsEMS.get(i).setHasAailableMule(true);
			}
			...
			for (int loopx = 0; loopx < Math.sqrt(NUM_Mules); loopx++) {
				for (int loopy = 0; loopy < Math.sqrt(NUM_Mules); loopy++) {
					Mule m = new Mule((1 + 2 * loopx) * MAX_X / (2 * Math.sqrt(NUM_Mules)),
							(1 + 2 * loopy) * MAX_Y / (2 * Math.sqrt(NUM_Mules)), this);
					mules.add(m);
					updateMinNodeDistance(m);
				}
			}
		} else if (Main.KMedianMode) {//
			System.out.println("placing " + mules.size() + " mules for centroid "+Main.CentroidAdjustmentMode);
	...
	//
	// System.out.println("re-placing "+NUM_Mules+" mules to centroid position
	// of their cluster");
	//
	// for (Mule m:mules){
	...
	public void rePlaceMules(double time) {
		...
		else if(Main.EMSMode){
			EMSDeploy(time); //not finished yet !!!
		}
		else {
			farthestFirst3(time);
		}
	...
	public void resetCloseNodes() {
		...
		if(MuleLocationsEMS!=null&&!MuleLocationsEMS.isEmpty()) {
			for (Pointxy p:MuleLocationsEMS){
				p.setNumberOfNodesClosestToMe(0);
			}
		}
	}
	...
	public void farthestFirst1(double time) {// this finds the farthest only
		// from newly placed mules not
		// from occupied
	...
	public void farthestFirst2(double time) {// the assignment of mules to nodes
		// is done optimally
		// also finds the farthest only from newly placed mules not from
		// occupied
		...
		ArrayList<Mule> inactiveMules = getInactiveMules(time);
		ArrayList<Mule> unPlacedMules = removePlacedMules(occupiedMules);
		ArrayList<Mule> VirtualPlacedMules = new ArrayList<Mule>();// used for
		// optimal
		// assignment
		...
		if (!occupiedMules.isEmpty()) { // find farthest from occupied
			...
			locationsToPlace.add(n); // add to locations sent to HA. it doesn't
			// really have to be a node but for the
			// allocation of HA it is needed
		...
		// Now create the distance matrix between (real) unplaced mules to the
		// location list
		...
		for (int i = 0; i < unPlacedMules.size(); i++) {
			System.out.print("Mule " + unPlacedMules.get(i).index + "<" + unPlacedMules.get(i).x + ","
					+ unPlacedMules.get(i).y + "> dist:");
		...
		for (int i = 0; i < unPlacedMules.size(); i++) {
			...
			System.out.println("placing mule " + m + " at node " + n + " at dist " + dist + " distMatrix="
					+ distanceMatrix[i][nodeIndex]);
	...
	public void farthestFirst3(double time) {// used for weighted kcenter
		//finds largest weight node for anchor
		System.out.println("running farthest3 at time :" + time);
		ArrayList<Mule> occupiedMules = getPlacedMules(time);
		ArrayList<Mule> inactiveMules = getInactiveMules(time);
		ArrayList<Mule> unPlacedMules = removePlacedMules(occupiedMules);//inactive are removed auto
		ArrayList<Mule> VirtualPlacedMules = new ArrayList<Mule>();// used for optimal assignment
		ArrayList<Node> availableLocations = new ArrayList<Node>();
		ArrayList<Node> locationsToPlace = new ArrayList<Node>();
		if (occupiedMules.size() == mules.size() - inactiveMules.size()) {
			System.out.println("No availables mules to place. returning null");
			return;
		...
		if (!Main.WFFMode&&!occupiedMules.isEmpty()) { // find farthest weighted from occupied
			Node farthest;
			if(Main.NewFFMode) {//closest
				 farthest = getClosestNode(occupiedMules.get(0));
			}
			else {
				 farthest = getFarthestNode(occupiedMules);
			}
			System.out.println("choosing Node :" + farthest + " as anchor");
		...
		} else {// or largest weight if no occupied //always in WFFMode
			Node mwn=getMaxWeightNode();
			System.out.println("choosing largest weight node :" + mwn+ " as anchor");
			//Node n = getClosestNode(unPlacedMules.get(0));
			locationsToPlace.add(mwn); // add to locations sent to HA. it doesn't
			// really have to be a node but for the
			// allocation of HA it is needed
			System.out.println("adding :" + mwn + " to destination list");
			VirtualPlacedMules.add(new Mule(mwn.x, mwn.y, this));
		}
	...
	private Node getMaxWeightNode() {
		Node max=nodes.get(0);
		for(Node n:nodes){
			if(n.getWeight()>max.getWeight()){
				max=n;
			}
		}
		return max;}
	...
	public void kMedian(double time) {
		...
		// remove the node that add the least to the sum of distances from
		// remaining nodes
		...
		System.out.println("inactiveMules.size()" + inactiveMules.size());
		System.out.println("mules.size()" + mules.size());
		...
		for (int i = 0; i < numberOfTimesToRemove; i++) {
			resetNodeDistances();
			updateMinNodeDistances(); // set for each node its minimal distance
			// to any test mule
	...
	// private void placeMulesAtTestMules1(ArrayList<Mule> unPlacedMules,double
	// time) {
	...
	////// //then sorts the min distances and places the one with the largest
	// min distance first
	...
	// test=testMules.get(loopi);
	// minDist=MAX_X+MAX_Y;
	// System.out.println("find closest distance from location "+ test+" to a
	// mule");
	//
	// for(int j = 0; j< unPlacedMules.size(); j++){
	...
	private Mule getMinCostNode(ArrayList<Mule> unPlacedMules, double time) {
		// this is a not so elegant brute force way of calculating sum of
		// distances for each test mule
	...
	private double calculateSumOfDistances() {
		...
		double sum = 0;
		for (int loopi = 0; loopi < NUM_Nodes; loopi++) {
			sum += nodes.get(loopi).getMinDistFromMules() * nodes.get(loopi).getWeight();// for
			// weighted
			// version
		}
	...
	public void localSearch(double time) {
		// This method places mules distributedly according to each ones best
		// local position which minimizes :
		...
		if (!occupiedMules.isEmpty()) {
			...
			int counter = 0;
			while (notConvergedMules.isEmpty() == false && counter < 50) {// counter
				// <
				// Main.NUM_Nodes
				resetNodeDistances();
				resetCloseNodes();
				...
				System.out.println("going over " + notConvergedMules.size() + " un converged mules "
						+ " notConvergedMules.isEmpty() " + notConvergedMules.isEmpty());

				for (int i = 0; i < notConvergedMules.size(); i++) {
					Mule m = notConvergedMules.get(i);

					System.out.println("calculate sum for current position of mule " + m.index);
					...
					for (int x = Math.max(0, (int) m.x - mr); x <= Math.min(MAX_X, (int) m.x + mr); x++) {
						for (int y = Math.max(0, (int) m.y - mr); y <= Math.min(MAX_Y, (int) m.y + mr); y++) {
							...
							if (sumOfDist < minSumOfDist) {
								minX = x;
								minY = y;
								minSumOfDist = sumOfDist;
							}
						}
					}
					System.out.println("min value position " + minX + " , " + minY + " minSumOfDist=" + minSumOfDist);
					...
					if (minX == m.x && minY == m.y) {
						...
						System.out.println("Mule " + m.index + " has converged and moved " + dist + " from "
								+ m.origin_LS_x + "," + m.origin_LS_y + " to " + m.x + "," + m.y);
						...
						if (Main.DSA_Mode) {
							...
							System.out.println("Mule " + m.index + " is moving to " + minX + " , " + minY);
							m.move(minX, minY);
						}
					}
	...
	public void EMSDeploy (double time) {
		//Goal - maintain maximum coverage with fixed locations - EMSLocations ArrayList 
		//Send mule with lowest number of nodes to replace occupied mule
		System.out.println("running EMSDeploy at time :"+time);

		ArrayList<Mule> occupiedMules=getUnavailableMules(time);
		if(occupiedMules.size()==mules.size()){
			System.out.println("No availables mules to place. returning null");
			return;
		}
		else {
			Pointxy maxUncoveredLocation=getMaxUncoveredLocation(time);
			if(maxUncoveredLocation==null) {
				System.out.println("No uncovered Locations - so no redeployment - impossible if there are less mules than locations");
				return;
			}
			Mule toRedeploy=getMinCoveringMule(time,maxUncoveredLocation);
			if(toRedeploy==null) {
				System.out.println("No available mule to redeploy");

				return;
			}
			System.out.println("redeploying "+toRedeploy +" to "+maxUncoveredLocation.getX() +" , "+maxUncoveredLocation.getY());

			toRedeploy.EMSLocationIndex=maxUncoveredLocation.getIndex();
			maxUncoveredLocation.setHasAailableMule(true);


			double dist=getDist(maxUncoveredLocation,toRedeploy);
			System.out.println("dist= "+dist);

			double delay=toRedeploy.getAvailabilityTime()-time;
			if(delay<0){delay=0;}
			double timeToNode =dist+delay;

			toRedeploy.setAvailabilityTime(time+timeToNode);

			toRedeploy.move(maxUncoveredLocation.getX(),maxUncoveredLocation.getY());

			toRedeploy.setTravelDistance(toRedeploy.getTravelDistance()+dist);
			System.out.println(toRedeploy+" traveled "+toRedeploy.getTravelDistance());
		}
	}
	...
	public void potentialField(double time) {
		...
		// Each mule calculates the potential force from all other neighboring
		// mules and nodes
		// it then moves to the location in its domain where the field value is
		// the strongest
		// for x iterations :
		// 1. for every unoccupied mule {
		// 2. for every value in its domain{
		// 3. calculates its angle and distances to all mules and nodes(can stop
		// at some dist)
		...
		while (notConvergedMules.isEmpty() == false && counter < Main.NUM_Nodes) {
			...
			for (int i = 0; i < notConvergedMules.size(); i++) {
				...
				double potential = calculatePotential(m, unPlacedMules);// calculates
				// the
				// force
				// vector
				// from
	...
	private double calculatePotential(Mule m, ArrayList<Mule> unPlacedMules) {
		...
		for (int i = 0; i < unPlacedMules.size(); i++) {
			if (!unPlacedMules.get(i).equals(m)) {
				...
				System.out.println("mule " + m1.index + "(" + m1.x + "," + m1.y + ")" + " added xForce=" + dx / dist
						+ " yForce=" + dx / dist);
				...
				System.out.println("MULE " + m1.index + "(" + m1.x + "," + m1.y + ")" + " updated xForce=" + xForce
						+ " yForce=" + yForce);
		...
		for (int i = 0; i < nodes.size(); i++) {
			...
			System.out.println("Node " + n.index + "(" + n.x + "," + n.y + ")" + " updated xForce=" + xForce
					+ " yForce=" + yForce);
	...
	private Node getFarthestNode(ArrayList<Mule> placedMules) {
		...
		for (Node n : nodes) {
			// System.out.println("checking node "+n);
			if (maxDist < n.getMinDistFromMules() * n.getWeight()) {// for
				// weighted
				// version
				maxDist = n.getMinDistFromMules();
				max = n;
			...
			if (m.isActive()) {
				...
				if (minDist > dist) {
					...
					if (min.getTravelDistance() > m.getTravelDistance()) {
						// System.out.println("changed to mule "+m+" due to
						// travel dist");
	...
	private Mule getClosestMule(Mule test, ArrayList<Mule> unplacedMules) {
		...
		for (Mule m : unplacedMules) {
			...
			if (minDist > dist) {
				...
				if (min.getTravelDistance() > m.getTravelDistance()) {
					// System.out.println("changed to mule "+m+" due to travel
					// dist");
	...
	public void updateClosestNodes() {
		System.out.println("updating closest nodes to all mules and EMSLocations");
		for (Mule m:mules){
			for (Node n:nodes){
				System.out.println("node "+n+" closest mule is "+n.getClosestMule());

				if(n.getClosestMule().index==m.index) {
					m.numberOfClosestNodes++;//for ems		
					System.out.println("adding node "+n.index+" to mule "+m.index+"  closest nodes" +m);

				}
			}
			MuleLocationsEMS.get(m.EMSLocationIndex).setNumberOfNodesClosestToMe(m.getNumberOfClosestNodes());
		}
	}
	...
	public void updateMinNodeDistance(Mule m) {
		...
		for (Node n : nodes) {
			...
			// System.out.println("checking node "+n +" from "+ m+ " dist = "+
			// dist+" current min dist = "+ n.getMinDistFromMules()+ "current
			// closest mule = "+n.getClosestMule());
			...
			// else{//for the case that m moved and the dist is now larger, we
			// must find a new closest mule
			// if( m==n.getClosestMule()){
			// Mule c=getClosestPlacedMule(n);
			// n.setClosestMule(c);
			// dist=getDist(n,c);
			// n.setMinDistFromMules(dist);
			// System.out.println("m==n.getClosestMule() dist="+dist +"from
			// "+c);
			//
			...
			// }
			// }
			// System.out.println("checking node "+n +" min dist = "+
			// n.getMinDistFromMules());
		}
	}
		...
		for (Node n : nodes) {
			...
			if (dist < n.getMinDistFromMules()) {
				...
				if (m == n.getClosestMule()) {
					...
					dist = getDist(n, c);
					n.setMinDistFromMules(dist);
					// System.out.println("m==n.getClosestMule() dist="+dist
					// +"from "+c);
	...
	public double getDist (Pointxy p, Mule m) {//ems
		//	System.out.println("calsulating distance between "+p + " and "+m);
		double dist=Math.sqrt(Math.pow((p.getX()-m.x),2)+Math.pow((p.getY()-m.y),2));
		return dist;
	}
	...
	public void setFailures() {
		...
		for (int loopi = 0; loopi < NumberOfFailures; loopi++) {
			failures.add(new Failure(nodes.get((int) (rand.nextDouble() * NUM_Nodes)),
					(int) (rand.nextDouble() * Main.TotalExperimentTime)));
	...
	public void setFailures1() { // increase the chance of repeated failures
		...
		ArrayList<Node> closeNodes = getCloseNodes(n, 5);//5
	...
	public Mule sendMuleA(Failure currentFailure) {// closest with return to
		// position
		...
		double dist = getDist(n, m);
		// for penalty check
		if (Main.PenaltyMode && (currentFailure.getPenaltyCost() < dist * currentFailure.getNode().getWeight())) {
			// do not go - too expensive
			//	System.out.println("cost=" + currentFailure.getPenaltyCost() + " < weightedDist="
			//			+ dist * currentFailure.getNode().getWeight());
			//		System.out.println("do not go - too expensive!");
			currentFailure.setPenaltyActivated(true);
			//	currentFailure.getNode().setDownDuration(Main.AvgMaxDownTime_F);
		} 
		//else {
	...
	public Mule sendMuleA1(Failure currentFailure) {// closest to original
		// position for no
		// cooperation no return
	...
	public Mule sendMuleC(Failure currentFailure) {// closest available
		...
		m.numberOfFixs += 1;
		if(MuleLocationsEMS!=null&&!MuleLocationsEMS.isEmpty()) {
			MuleLocationsEMS.get(m.EMSLocationIndex).setHasAailableMule(false);
		}
		//	}
		return m;
	}
	...
	//
	// }
	private ArrayList<Mule> getUnavailableMules (double time) {//ems

		System.out.println("finding Unavailable mules at time :"+time);
		ArrayList<Mule> unavailableMules=new ArrayList<Mule>();
		for(Mule m:mules){

			if(m.getAvailabilityTime()>time){
				unavailableMules.add(m);
				System.out.println( m + " unavailable at time :"+time);

			}
		}
		if(unavailableMules.isEmpty()){
			//		System.out.println("mules");
			//		printArray(mules);
			System.out.println("no mules unavailable at time :"+time+ " returning null ");

		}
		return unavailableMules;
	}



	public Mule getMinCoveringMule(double time, Pointxy maxUncoveredLocation) {//for ems
		int maxUncoveredNodes=maxUncoveredLocation.getNumberOfNodesClosestToMe();
		System.out.println("getting a mule with the lowest number of nodes closest to it, lower than "+maxUncoveredNodes);
		Mule min=null;
		for (Mule m:mules){
			if( m.isAvailable(time)){
				if(m.numberOfClosestNodes<maxUncoveredNodes) {
					min=m;
				}
			}
		}
		if(min==null) {
			System.out.println("No mule found ");
		}
		else {
			{
				System.out.println("Sending "+min);
			}
		}
		return min;
	}
	public Pointxy getMaxUncoveredLocation(double time) {//for ems
		//note - we mark locations as occupied when a mule is on the way to prevent 2 mules going to the same location
		System.out.println("getting the unoccupied location with the max number of nodes closest to it at time "+time);
		Pointxy maxUncoveredLocation=null;
		int maxUncoveredNodes=0;
		for (Pointxy p:MuleLocationsEMS){
			//		System.out.println("checking location "+p);

			if( p.hasAailableMule()==false && p.getNumberOfNodesClosestToMe()>maxUncoveredNodes){
				maxUncoveredLocation=p;
			}
		}
		return maxUncoveredLocation;
	}
}
